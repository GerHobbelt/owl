   ...
   (begin
      ...
      (define (terminal-input . opt)
       (let ((port (if (null? opt) stdin (car opt))))
        (let loop ((ll (utf8-decoder (port->byte-stream port) (λ (loop line ll) (print-to stderr "Bad UTF-8 in terminal input") (loop line ll)))))
          (cond
            ((pair? ll)
              (lets ((hd ll ll))
                (cond
                  ...
                  ((eq? hd 6)  (cons (tuple 'ctrl 'f) (loop ll)))
                  ((eq? hd 9)  (cons (tuple 'tab) (loop ll)))
                  ((eq? hd 2)  (cons (tuple 'ctrl 'b) (loop ll)))
                  ((eq? hd  8)  (cons (tuple 'ctrl 'h) (loop ll)))
                  ((eq? hd 18)  (cons (tuple 'ctrl 'r) (loop ll)))
                  ((eq? hd 14)  (cons (tuple 'ctrl 'n) (loop ll)))
                  ((eq? hd 16)  (cons (tuple 'ctrl 'p) (loop ll)))
                  ((eq? hd 11)  (cons (tuple 'ctrl 'k) (loop ll)))
                ...
                ((backspace)
                  (if (= cx x) ;; beginning
                    (if (= off 0) ;; no scroll, do nothing
                      ;(values ll #false) ;; exit on backspace of empty ;; todo: add a way to configure these
